...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-4 mt-6 pb-12">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-white p-4 rounded-xl shadow-sm border"><p class="text-sm text-gray-500">รายรับค่าห้อง</p><h3 class="text-2xl font-bold text-sky-600 mt-1">฿<?= number_format($total_rent_income, 2) ?></h3></div>
            <div class="bg-white p-4 rounded-xl shadow-sm border"><p class="text-sm text-gray-500">รายรับค่าไฟ</p><h3 class="text-2xl font-bold text-amber-600 mt-1">฿<?= number_format($total_electric_income, 2) ?></h3></div>
            <div class="bg-white p-4 rounded-xl shadow-sm border"><p class="text-sm text-gray-500">รายรับ (น้ำ+ไฟ)</p><h3 class="text-2xl font-bold text-cyan-600 mt-1">฿<?= number_format($total_utility_income, 2) ?></h3></div>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <!-- Form Section -->
            <div class="lg:col-span-1 bg-white p-5 rounded-xl shadow-sm border">
                <h3 class="text-lg font-bold text-gray-800 mb-4"><i class="fa-solid fa-plus-circle text-indigo-500"></i> สร้างบิลค่าเช่าใหม่</h3>
                <form method="POST" action="rent_bill.php" class="space-y-4 text-sm" id="rent-form">
                    <input type="hidden" name="action" value="save_bill">
                    <input type="hidden" name="bill_month" value="<?= $month ?>">
                    
                    <div><label class="font-medium">เลขห้อง</label><input type="text" name="room_number" placeholder="เช่น 101" class="w-full mt-1 px-3 py-2 border rounded-lg" required></div>
                    <div><label class="font-medium">ค่าเช่าห้อง</label><input type="number" step="0.01" name="rent_amount" id="rent_amount" placeholder="เช่น 5000" class="w-full mt-1 px-3 py-2 border rounded-lg" required></div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div><label class="font-medium">มิเตอร์ไฟ (ก่อน)</label><input type="number" step="0.01" name="prev_meter" id="prev_meter" placeholder="หน่วยเก่า" class="w-full mt-1 px-3 py-2 border rounded-lg" required></div>
                        <div><label class="font-medium">มิเตอร์ไฟ (หลัง)</label><input type="number" step="0.01" name="current_meter" id="current_meter" placeholder="หน่วยใหม่" class="w-full mt-1 px-3 py-2 border rounded-lg" required></div>
                    </div>
                    
                    <div><label class="font-medium">หมายเหตุ</label><textarea name="notes" rows="2" placeholder="รายละเอียดเพิ่มเติม (ถ้ามี)" class="w-full mt-1 px-3 py-2 border rounded-lg"></textarea></div>
                    ...
                    <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-lg font-medium text-base">บันทึกบิล</button>
                </form>
            </div>
                    ...
                    <table class="w-full text-left text-sm">
                        <thead class="bg-gray-100 text-gray-600 uppercase text-xs">
                            <tr>
                                ...
                                <th class="p-3">ค่าเช่า</th>
                                <th class="p-3">ค่าน้ำ+ไฟ</th>
                            ...
                            <?php else: ?>
                                <?php foreach ($bills_for_month as $bill): ?>
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="p-3">฿<?= number_format($bill['rent_amount'], 2) ?></td>
                                        <td class="p-3">฿<?= number_format($bill['water_bill'] + $bill['electricity_cost'], 2) ?></td>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function calculateTotal() {
                ...
                if (currentMeter > prevMeter) {
                    electricity = (currentMeter - prevMeter) * 7;
                }
